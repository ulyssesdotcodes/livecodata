...
const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  beats:      { sig: 'beats(count, { fit }?)',       detail: 'beat timeline',     info: 'A looping timeline `count` beats long; its wall-clock length follows the tapped tempo (🥁 Tap under the scene). Use as the "timeline" view: define("timeline", () => beats(16)). { fit: beats } stretches a span of source beats across the window.' },
