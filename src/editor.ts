...
const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  retime:      { sig: '.retime({ ease }?)',                   detail: 'keyframe time-warp', info: 'Rasterize time-mapping keyframe rows { src, dst, ease? } (source seconds → playback seconds, eased to the next keyframe) into a dense "timeline" view. ease: a function or an easing name ("easeInOut").' },
  pace:        { sig: '.pace({ until, speed, ease }?)',       detail: 'speed between events', info: 'Build a "timeline" view that warps playback speed between this table\'s events (any table with a time field — default "index", seconds). speed per segment: a number, a cycling array ([0.4, 2] alternates slow/fast), or (seg, {from,to}) => number. until: source length in seconds.' },
