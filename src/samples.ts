...
//                       per-pixel mask, \`gradient(0).thresh(progress())\` — a
//                       directional wipe. The mask is a black→white chain (black
//                       keeps the old chain, white shows the new); it moves ONLY
//                       because \`progress()\` sweeps 0→1 across the window (this
//                       beat → the destination's beat, shaped by \`ease\`).
//                       \`progress()\` alone would crossfade; leave \`code\` BLANK
//                       and the mask is static black — the old chain HOLDS the
//                       whole window, then cuts to the new one (a delayed cut).
...
`,
    tables: {
      post: [
        ...
        { beat: 13, event: "transition", code: "gradient(0).thresh(progress())", ease: "easeInOut" },
