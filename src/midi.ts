...
// makes them loop-friendly: each event is stamped with the playhead's *content/
// source* position at the moment it arrived (seconds — see
// Playback.currentSourceSeconds), not wall-clock time. That's the same
// coordinate the whole baked scene is keyed to, so a recorded sweep's speed
// tracks the timeline mapping: map the timeline to run 2x slower and a sweep
// that took 1s to record now takes 2s to play back, right along with everything
// else on screen — whether or not the timeline was already mapped when the note
// was played, since the stamped value is always this shared content coordinate,
// never the mapping used to reach it.
//
...
// each frame against this table (see resolveBindings in dsl.ts), sampled at the
// same source frame events are recorded at.
// ----------------------------------------------------------------------------
...
export interface MidiInput {
  ...
  // recorded table at `srcFrame` (the same content/source domain events are
