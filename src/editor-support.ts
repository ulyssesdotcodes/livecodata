...
export const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  origami:    { sig: 'origami() | origami.crane() | origami.fan(n)', detail: 'folding paper', info: 'A sheet of paper that folds live. Chain .crease(x1,y1,x2,y2, group, angle) to add fold lines (degrees; + valley toward you, − mountain), .spawn({ id, color, … }) for the scene create row, and .sequence(steps) to bake { fold, at, dur, to, ease } steps into beat-timed keyframes. Fold state is plain numeric fields (0 flat → 1 folded), so scrubbing backwards physically unfolds the paper.' },
