...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  steps:       { sig: '.steps(table | rows)',                 detail: 'origami instructions', info: 'One table = the whole folding, written as instructions: each row folds ("op": "fold", by `deg` degrees, moving just the flap connected to `move`) or reflects ("reflect", a flat 180° fold through every layer on one side) the paper along the line through `p1` and `p2`. Points: "x,y" a paper point in sheet coordinates, "name@t" a fraction t along the edge created by the earlier fold `name`. `dir` +1 folds toward you, −1 away; `at`/`dur` time it. A row with timing but no line re-drives an earlier fold by name.' },
