...
export const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  origami:    { sig: 'origami() | origami.fan(n)', detail: 'folding paper', info: 'A sheet of paper that folds live. Chain .folds(table) to drive it from a table of physical fold steps (each row a crease line + degree + timing), or .crease()/.creases() to add fold lines directly; then .spawn({ id, color, … }) for the scene create row and .sequence() for the beat-timed keyframes. Folding is simulated as paper (a physics solver after Ghassaei\'s Origami Simulator) and baked against the beat, so playback and scrubbing land on the same pose every time.' },
...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  crease:      { sig: '.crease(x1, y1, x2, y2, group, angle)', detail: 'origami fold line', info: 'Add a crease to an origami sheet: a line (clipped to the paper, split at crossings) in a named group with a target fold angle in degrees (+ valley toward you, − mountain).' },
  creases:     { sig: '.creases([{ x1,y1,x2,y2,group,angle }, …])', detail: 'origami fold lines', info: 'Add many creases at once from a plain array of objects — the declarative counterpart of chaining .crease() repeatedly, for a whole crease pattern authored as data.' },
  folds:       { sig: '.folds(table | steps)',                detail: 'origami fold steps',  info: 'One table = the whole folding, written like physical origami steps: each row is a crease LINE in original-sheet coordinates { x1,y1,x2,y2 } plus a degree (deg; + toward you, − away) and its timing { at, dur, to }. Rows sharing a `fold` name move together; a row with timing but no line re-folds an earlier step by name. The lines join the crease pattern and the timings become sequence()\'s default steps.' },
  spawn:       { sig: '.spawn({ id, color, px, … })',         detail: 'origami create row', info: 'The origami sheet\'s scene create row (shape: "origami"), with every fold group initialised to 0 (flat). Props merge over defaults. Any schedule captured by .folds() rides along as `drive`, and the renderer pre-simulates (bakes) the whole folding against the beat.' },
  sequence:    { sig: '.sequence(steps?)',                    detail: 'origami schedule',  info: 'Bake fold steps { fold, at, dur?, to?, ease? } into beat-timed update keyframes for the sheet spawned from this builder. With no argument, uses the steps captured by .folds(). Steps may overlap; `to` below 1 part-folds, back to 0 unfolds; ease is an easing fn or name.' },
