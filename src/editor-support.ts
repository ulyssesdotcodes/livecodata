...
export const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  origami:    { sig: 'origami()', detail: 'folding paper', info: 'A sheet of paper folded by instructions. Chain .steps(table) — each row folds or reflects the paper along a line through two points on known edges ("x,y" a paper point in sheet coordinates, "name@t" a point along the edge created by the earlier fold `name`) — then .spawn({ id, color, … }) for the scene create row and .sequence() for the beat-timed keyframes. Folding is exact kinematics: each step rigidly rotates the faces it moves about its fold line, so scrubbing shows the exact fold state at any beat.' },
...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  steps:       { sig: '.steps(table | rows)',                 detail: 'origami instructions', info: 'One table = the whole folding, written as instructions: each row folds ("op": "fold", by `deg` degrees, moving just the flap connected to `move`) or reflects ("reflect", a flat 180° fold through every layer on one side) the paper along the line through `p1` and `p2`. Every position is on a known edge: "bottom@t"/"top@t"/"left@t"/"right@t" a fraction along that paper edge, "name@t" along the edge created by the earlier fold `name`. `dir` +1 folds toward you, −1 away; `at`/`dur`/`to` time it (1 folded, 0 open, −1 folded the other way). A row re-using a fold\'s name re-drives it — whole with no line, or just the stretch between p1/p2 given on its own edge.' },
