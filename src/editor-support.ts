...
export const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  three:      { sig: 'three.box / .sphere / .points / .camera / .translate …', detail: 'three.js namespace', info: 'The three.js helpers, grouped under one namespace (shorthand: t). Scene primitives three.box/sphere/cylinder/cone/torus/text and the generic three.object(shape, props) each build a "create"-row Table — beat 1, at the origin — to concat and .rasterize(). three.points(shape, props) ⇄ three.geometry(points) sample a primitive to point rows and back. three.camera([keyframes]) emits beat-timed camera moves. three.translate/scale/rotate(table, x, y, z) shift a scene table\'s create rows in place. e.g. t.box({ color: 0x4a9eff }).concat(t.sphere({ px: 1 })).rasterize(8).' },
...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  rasterize:   { sig: '.rasterize(maxBeats?)',                detail: 'bake frame cache', info: 'Bake sparse event rows (keyed by `beat`) into a dense per-frame world state Table. Optional maxBeats bakes at least that many beats (the final pose holds to it); omitted, the cache sizes to the last event. The loop itself is the "beats" control under the scene — content past it plays in later passes of the loop.' },
