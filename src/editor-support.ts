...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  steps:       { sig: '.steps(table | rows)',                 detail: 'origami crease table', info: 'One table = the whole folding, as a static crease list. Each row with a line is a crease: `p1`/`p2` its segment in sheet coordinates ("x,y"), `move` sample points inside the pieces it rotates (";"-separated; only pieces touching the crease), `sign` the crease\'s turning sense (flip it — or swap p1/p2 — if a flap tears away mid-fold), `deg` the full signed angle (±180 = flat), and `at`/`dur`/`to` its timing (1 folded, 0 open, −1 folded the other way; dur 0 = geometry only). Rows sharing a `step` name extend one fold across several layers; a row with no line re-drives an earlier fold (keyframes).' },
