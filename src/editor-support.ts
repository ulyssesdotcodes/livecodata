...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  outThree:    { sig: '.outThree()',                          detail: 'route to 3D scene', info: 'Send this table of scene event rows to the 3D scene — no define() or name needed: t.box().three.rotate().outThree(). Every .outThree() table (plus a view named "three", if any) is combined beat-sorted into the "three (system)" table, visible in the panel and rasterized for playback. With no .outThree() anywhere, the scene reads the table named "three" as before.' },
  outScene:    { sig: '.outScene()',                          detail: 'route to scene cache', info: 'Send PRE-RASTERIZED per-frame rows to the scene cache — for when you call .rasterize() yourself. Combined with every other .outScene() table (and a "scene" view, if any) into "scene (system)". Usually .outThree() is what you want.' },
  outHydra:    { sig: '.outHydra()',                          detail: 'route to hydra',    info: 'Send this table of hydra event rows (schemas.hydra) to the hydra video synth. Every .outHydra() table (plus a view named "hydra", if any) is combined beat-sorted into "hydra (system)", which playback reads. With no .outHydra() anywhere, the table named "hydra" is read as before.' },
  outBauble:   { sig: '.outBauble()',                         detail: 'route to bauble',   info: 'Send this table of bauble event rows (schemas.bauble) to the SDF renderer, combined with every other .outBauble() table (and a "bauble" view, if any) into "bauble (system)".' },
  outPost:     { sig: '.outPost()',                           detail: 'route to post fx',  info: 'Send this table of post event rows (schemas.post) to the post-processing chain, combined with every other .outPost() table (and a "post" view, if any) into "post (system)".' },
  outTimeline: { sig: '.outTimeline()',                       detail: 'route to timeline', info: 'Send this table of timeline event rows (schemas.timeline) to the playback timeline — e.g. beats(16).outTimeline(). Combined with every other .outTimeline() table (and a "timeline" view, if any) into "timeline (system)".' },
  outParticles:{ sig: '.outParticles()',                      detail: 'route to particles', info: 'Send this table of particle event rows (schemas.particles) to the GPU particle sim, combined with every other .outParticles() table (and a "particles" view, if any) into "particles (system)".' },
}
