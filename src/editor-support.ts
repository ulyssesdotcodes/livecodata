...
export const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  origami:    { sig: 'origami() | origami.fan(n)', detail: 'folding paper', info: 'A sheet of paper that folds live. Chain .folds(table) to drive it from a table of physical fold steps (each row a crease line + degree + timing), or .crease()/.creases() to add fold lines directly; then .spawn({ id, color, … }) for the scene create row and .sequence() for the beat-timed keyframes. Folding is kinematic — every crease sits at exactly its scheduled angle, faces stay rigid, the sheet is stitched so edges stay together, and scrubbing shows the exact fold state at any beat.' },
