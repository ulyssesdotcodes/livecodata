...
// A real Jolt engine, loaded once and shared across the simulation tests below.
// The asm.js build takes a moment to spin up, so do it ahead of the tests.
const engine = await initPhysics()
const Jolt = engine.Jolt

function rowsById(out, id) {
...
test('a dynamic body falls under gravity', () => {
  ...
  assert.ok(last.py < first.py - 1, `ball fell: ${first.py.toFixed(2)} -> ${last.py.toFixed(2)}`)
})
...
test('output is sorted by index, base creates first at index 0', () => {
  ...
  for (let i = 1; i < out.length; i++) {
    assert.ok((out[i].index ?? 0) >= (out[i - 1].index ?? 0), 'non-decreasing index')
  }
...
test('sampleEvery thins out the update rows', () => {
  const base = [{ id: 'ball', type: 'create', shape: 'sphere', motion: 'dynamic', px: 0, py: 5, pz: 0 }]
...
test('initial velocity is applied to dynamic bodies', () => {
  ...
  assert.ok(updates[updates.length - 1].px > 1, 'travelled along +x from its initial velocity')
})

// ── DSL wiring (runtime, no real engine needed — uses a fake) ────────────────
