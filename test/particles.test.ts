...
test('particles: a spawn row opts the sim in; disabled rows do not', () => {
  assert.equal(hasSpawner(particleRows([{ event: 'set', name: 'speed', value: 1 }])), false)
...
test('particles: set rows fold at-or-before the playhead, last write wins, unknown names ignored', () => {
  const rows = particleRows([
    { beat: 1, event: 'spawn' },
    { beat: 1, event: 'set', name: 'speed', value: 0.001 },
    { beat: 5, event: 'set', name: 'speed', value: 0.01 },
    { beat: 5, event: 'set', name: 'elscale', value: 8 },
    { beat: 9, event: 'set', name: 'nonsense', value: 42 },
  ])
