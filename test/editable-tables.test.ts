...
test('an edit while replaying returns to head, never rewriting history', () => {
  ...
  store.createTable('nums')
  store.addRow('nums')
  store.setCell('nums', 0, 'index', 1)
  const run1 = store.recordRun()
  store.setCell('nums', 0, 'index', 2)
  store.recordRun()

  store.setReplayView(run1)
  assert.equal(store.get('nums')!.rows[0].index, 1, 'viewing the past')
  // Editing lands on the head (index 2 → 3), not on the replayed past.
  store.setCell('nums', 0, 'index', 3)
  assert.equal(store.get('nums')!.rows[0].index, 3)
