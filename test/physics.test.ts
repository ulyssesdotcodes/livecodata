...
test('a dynamic body falls under gravity', () => {
  const out = simulateScene(Jolt, [
    { id: 'ball', event: 'create', shape: 'sphere', motion: 'dynamic', px: 0, py: 5, pz: 0 },
  ], { steps: 60, gravity: -9.81 })

  const updates = rowsById(out, 'ball').filter((r) => r.event === 'update')
...
test('dropAt holds a dynamic body motionless, then releases it into free fall', () => {
  ...
  const out = simulateScene(Jolt, [
    { id: 'ball', event: 'create', shape: 'sphere', motion: 'dynamic', dropAt: 1, px: 0, py: 5, pz: 0 },
...
test('static bodies are preserved but never emit update rows', () => {
  const out = simulateScene(Jolt, [
    { id: 'floor', event: 'create', shape: 'box', motion: 'static', px: 0, py: -1, pz: 0, hx: 5, hy: 0.5, hz: 5 },
    { id: 'ball', event: 'create', shape: 'sphere', motion: 'dynamic', px: 0, py: 3, pz: 0 },
  ], { steps: 30 })

  const floorRows = rowsById(out, 'floor')
  assert.equal(floorRows.length, 1, 'floor appears exactly once (its create row)')
  assert.equal(floorRows[0].event, 'create')
  assert.ok(rowsById(out, 'ball').some((r) => r.event === 'update'), 'ball still moves')
})
...
test('a body landing on the floor produces collision rows for both bodies', () => {
  ...
  const collisions = out.filter((r) => r.event === 'collision')
...
test('collisions:false skips contact rows', () => {
  ...
  assert.equal(out.filter((r) => r.event === 'collision').length, 0)
})
...
test('output is sorted by beat, base creates first at beat 1', () => {
  ...
  assert.equal(out[0].event, 'create')
  assert.equal(out[0].beat, 1, 'create normalized to beat 1')
})

test('sampleEvery thins out the update rows', () => {
  const base: Row[] = [{ id: 'ball', event: 'create', shape: 'sphere', motion: 'dynamic', px: 0, py: 5, pz: 0 }]
  const dense = simulateScene(Jolt, base, { steps: 60, sampleEvery: 1 })
  const sparse = simulateScene(Jolt, base, { steps: 60, sampleEvery: 4 })

  const denseUpdates = dense.filter((r) => r.event === 'update').length
  const sparseUpdates = sparse.filter((r) => r.event === 'update').length
...
test('simulation is deterministic across runs', () => {
  const base: Row[] = [
    { id: 'a', event: 'create', shape: 'box', motion: 'dynamic', px: 0.1, py: 4, pz: 0, rx: 0.3 },
    { id: 'b', event: 'create', shape: 'sphere', motion: 'dynamic', px: 0, py: 2, pz: 0 },
    { id: 'g', event: 'create', shape: 'box', motion: 'static', px: 0, py: -1, pz: 0, hx: 3, hy: 0.3, hz: 3 },
  ]
...
test('initial velocity is applied to dynamic bodies', () => {
  const out = simulateScene(Jolt, [
    { id: 'shot', event: 'create', shape: 'sphere', motion: 'dynamic', px: 0, py: 0, pz: 0, vx: 5 },
  ], { steps: 30, gravity: 0 })

  const updates = rowsById(out, 'shot').filter((r) => r.event === 'update')
...
test('physics() builder bakes the source table and stays chainable', () => {
  const baked: Row[] = [
    { id: 'x', event: 'create', beat: 1 },
    { id: 'x', event: 'update', beat: 2, py: 1 },
  ]
  const runtime = createRuntime({ physics: () => fakeEngine(baked) })
  const { views } = runtime.run(`
    define("scene", () => rows([{ id: "x", event: "create", motion: "dynamic" }]))
...
test('physics() throws a friendly error while the engine is still loading', () => {
  ...
  assert.throws(() => runtime.run(`
    define("three", (rand, table) => physics(rows([{ id: "x", event: "create" }])).simulate())
