...
// Convenience: convert frame count to seconds for API calls (FPS 60).
const f = (frames) => frames / 60

// Resolve the chain from raw effect rows at a given frame, in one call.
const chainAt = (rows, frame) => effectChainAtFrame(buildEffectIndex(rows), frame)
...
test('addEffect activates an effect with type defaults merged under its params', () => {
  ...
  // Overridden param wins; unset params fall back to EFFECT_TYPES defaults.
...
test('chain orders effects by input wiring: base reader first, then downstream', () => {
  // Declared out of chain order on purpose — ordering must follow `input`.
  const chain = chainAt([
